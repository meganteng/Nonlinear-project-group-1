function plot_observer_states(ts, xs, xhat, ref_ps, ref_vs, theta_ds)
...
hold on;
plot(ts, 100 * xhat(1, :),':', 'LineWidth', 1.5);
plot(ts, 100 * ref_ps, '-.', 'LineWidth', 1.5);
ylabel('$z\hat$ [cm]', 'Interpreter', 'latex');
grid on;
title('State History');
...
hold on;
plot(ts, 100 * xhat(2, :),':', 'LineWidth', 1.5);
plot(ts, 100 * ref_vs, '-.', 'LineWidth', 1.5);
grid on;
ylabel('$\dot{z\hat}$ [cm / s]', 'Interpreter', 'latex');

subplot(4, 1, 3);
plot(ts, 180 * xs(3, :) / pi, 'LineWidth', 1.5);
hold on
plot(ts, 180 * xhat(3, :) / pi,':', 'LineWidth', 1.5);
...
hold on
plot(ts, 180 * xhat(4, :) / pi,':', 'LineWidth', 1.5);
